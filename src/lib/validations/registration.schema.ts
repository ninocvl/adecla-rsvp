...
export const PADEL_PARTICIPANT_TYPES = [
  "AFILIADO",
  "PATROCINADOR",
  "CLUB",
  "PUBLICO",
...
// Pádel pregunta con qué situación llega el jugador (padelParticipantType):
// afiliado de ADECLA (reusa isAffiliated/affiliateId), invitado de un
// patrocinador (isSponsorGuest, sin costo), miembro de un club con convenio
// (padelClub, 20% de descuento), o público general (sin nada especial). El
// servidor vuelve a leer el evento real por eventId antes de confiar en
// esto: el eventSlug de aquí solo determina qué validar en el formulario.
const companyFieldsSchema = z.object({
  eventSlug: z.string().min(1),

  // --- Golf: membresía ADECLA. También lo reusa pádel para AFILIADO. ---
  ...
  // --- Pádel ---
  padelParticipantType: z.enum(PADEL_PARTICIPANT_TYPES).optional(),
...
function validateCompanyFields(
  ...
  if (data.eventSlug === "padel") {
    ...
    if (data.padelParticipantType === undefined) {
      ctx.addIssue({
        code: "custom",
        message: "Indica tu situación para la inscripción.",
        path: ["padelParticipantType"],
      });
      return;
    }
    if (data.padelParticipantType === "AFILIADO" && !data.affiliateId) {
      ctx.addIssue({
        code: "custom",
    ...
    if (data.padelParticipantType === "PATROCINADOR") {
    ...
    if (data.padelParticipantType === "CLUB" && !data.padelClub) {
      ctx.addIssue({
        code: "custom",
        message: "Selecciona tu club.",
        path: ["padelClub"],
      });
    }
    return;
